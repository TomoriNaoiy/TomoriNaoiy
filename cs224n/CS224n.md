...
# L1
## one-hot
独热编码 一种词向量获得方式 由0和1组成 但是具有过于离散并且词语关联性不强的缺点（词嵌入就是词向量）
...
简略来说 我们就是利用softmax计算词j出现在词i上下文的概率 并且同样使用交叉熵计算损失
...
不过这种类比法a:b c:d的方法 通常我们用作网络的内部评估

以次评估系统的效果 从而对参数进行调优

nlp和传统深度网络调优的区别 我们在数据集大的时候 我们可以通过重新训练来提高效果 当然 数据集小就没有意义了

# 两种评估
刚才提到了内部评优 我们还有外部评优 简单来说就是根据预测的实际效果来评估 内部评估就是通过词与词类比的方法来评估效果
# L2
上文提到skip-gram算法
...
我们还有另一种简单的实现词向量的方式 **共现矩阵** 虽然也并非最优 但确实从另一个角度来运算词向量
...
<img width="103" height="44" alt="image" src="https://github.com/user-attachments/assets/97cdccca-da80-4462-bf38-afb2444b0f4b" />
...
这样的实现 统合了两者的有点 在大部分的训练中比较具有优势 运用也更普遍
 ...
 tip:GloVe 中使用中心词左右的窗口大小为 8 时模型表现较好。
...
# 数据处理
同样的 nlp中出现过拟合的情况非常常见 我们依旧可以引入L2正则化 进行惩罚’
...
### seq2seq架构
seq2seq既可以说是一种网络 也可以说是一种架构 

做为nlp中重要的网络架构 他主要作用是将一种句子翻译成另一种句子 （比如英语->法语）

明确的说 其实就是编码器-解码器架构 （encoder-decoder-）

**Seq2Seq 是一个有两个循环神经网络组成的端到端模型：**

- 一个编码器，将模型的输入序列作为输入，然后编码固定大小的“上下文向量”。-
- 一个解码器，使用来自解码器生成的上下文向量作为从其生成输出序列的“种子”

像我们的大名鼎鼎的transformer就是使用的encoder-decoder架构

其中衍生的 Transformer、Google 的 T5、Meta 的 BART。

擅长：机器翻译 摘要生成等等

擅长： 典型的 Seq2Seq 任务，比如机器翻译、长文总结摘

还有其中的变体 **BERT** 等 运用的是encoder-only架构 也就是只有半边  

擅长： 阅读理解、完形填空、情感分析、文本分类。

然后**GPT-3，Qwen** 等 运用的是decoder-only架构

擅长： 聊天对话、写文章、写代码、续写故事。

# 顺带一提 当前业界内几乎所有的大模型 gpt gemini grok 豆包还是kimi 都是使用的decoder-only架构
