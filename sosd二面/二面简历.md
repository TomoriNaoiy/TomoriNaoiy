...
# Task1 transformer
**优势**：
1. 并行计算能力强 相较于传统的Rnn循环神经网络 使用了自注意力机制 区别于rnn的逐步计算 能够同时对整个序列进行注意力机制的计算 因此可以实现并行计算
2. 长距离依赖建模能力强 rnn每一步的计算依赖与前一步的信息 在面对长距离的计算时 可能会遗忘过早的信息 并且会出现梯度消失/爆炸的问题 而transformer使用的多头注意力机制 对整个信息进行处理 因此对于长距离序列解决能力强
3. 泛化性好 现在市面上的大部分nlp,cv,llm都是基于transformer架构
### 最大优势：并行计算能力强
相对于rnn长距离计算上的缺陷 在rnn基础上增加的lstm 加入了各种门 能够一定程度上解决长距离处理上的缺陷 而transformer不仅在长距离计算上颇具优势 而且他的强大的并行计算能力才是真正让他成为热门框架 抛弃了逐步计算的方式，可以一次性并行处理整个序列。这一点对训练效率的提升是量级上的飞跃，尤其在大数据和 GPU/TPU 训练时代显得极其重要。正是因为并行计算，Transformer 才能在可接受的时间内训练超大规模模型 比如基于transformer架构的gpt以及bert

**缺点**：
1. 计算复杂度高：自注意力是 O（n²）序列越长计算越慢。
2. 资源消耗大：训练需要海量数据和算力，小规模训练容易过拟合。
### 最大缺点 ：计算复杂度高
由于O（n²）的时间空间复杂度 使其在推理的过程 速度会很慢 而且这是自注意力机制最本质的缺点 需要大量数据的缺点可以通过预训练以及迁移学习等方式解决 但是时间复杂度的问题是transformer自注意力机制算法的最本质的问题 这也是将来需要的优化方向
...
<img width="1659" height="511" alt="image" src="https://github.com/user-attachments/assets/941ebfa0-bd99-491a-a8e9-e81df4a0738a" />
...
每个子层外包 Residual + LayerNorm。（残差和norm）
...
```
### 问题3
因为自注意机制实际上就是对每个词和句子中的其他词计算相似度 如果只用一种头 那么只能关注单一的特征 如果使用多个头 就可以从多个方面进行求相似度 比如从语义 语法 位置等等 如果使用单一head可能会导致信息平均化 因此使用多头注意力可以表示能力干好 使得模型的效果更好

### 问题4
使用位置编码 便于体现词向量之间的相对距离
<img width="579" height="158" alt="image" src="https://github.com/user-attachments/assets/7329c948-05cc-441a-8282-383b53381196" />
找到计算公式
...
```
### 问题5
每层还有一个点式前馈 FFN。他是两个线性层和一个非线性层 对每个token位置独立且相同参数地应用 并且不像注意力机制没有交叉，就是simple的神经网络（所以叫 position-wise），本质上相当于对每个位置做相同的 MLP 对特征进行处理 从而增强模型的实现能力
...
- 问题1 论文的变体试验显示：把注意力头论文从8减少到 1 时，BLEU 会下降（论文提到 single-head 比最好设置差约 0.9 BLEU，实验中头数减少导致 BLEU 降，是直接的、可量化的证据，说明多头确实提升了模型的表示与翻译质量。从理论上来说 减少注意力头数会使得对特征的捕捉能力下降，信息被泛化 因此最终导致了BLEU下降
- 问题2 论文的实验组（表 3 的 (B) 组）表明把 key 的维度 d_k 缩小会损害模型质量，使得投影的子空间过窄，说明 attention 中确定兼容性的函数不容易，较小的 d_k会限制了表达能力。
 ...
 **值得一提的是 本人有记笔记的习惯 因此专门写一篇note记录一些自己对这篇论文的理解 也供给以后回顾和加深印象 ["AIAYN论文note"](https://github.com/TomoriNaoiy/TomoriNaoiy/blob/main/sosd%E4%BA%8C%E9%9D%A2/Attention%20is%20all%20you%20need%E8%AE%BA%E6%96%87%E7%B2%BE%E8%AF%BB.md)**
